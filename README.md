<p align="center">
  <img src="https://raw.githubusercontent.com/ul0gic/ul0gic/main/29f5ba1fb4acbd516e4cbc88aad1b16dc054766c9b53692092e6410e53d706d9.png" alt="ul0gic banner" width="100%">
</p>

# ⚙️ ul0gic

### Security Architect • Backend Engineer • Offensive Builder

I design, break, and rebuild systems until they perform flawlessly — or burn beautifully.  
Strong bias toward **security**, **backend architecture**, and **high-performance engineering**.

---

### 🧠 Core Focus

- Offensive & defensive security tooling  
- Backend systems in **Rust**, **Go**, and **Python**  
- Data analytics, quick-hit automation, and scraping  
- Self-hosted infrastructure, **FreeBSD** minimalism, and **AWS / Azure** scaling  
- DevSecOps mindset — **Biome**, **Knip**, and static analysis everywhere

---

### 🧩 Projects

| Project | Domain | Core Stack |
|----------|---------|------------|
| **Discredit** | Security / OffSec Toolkit | Rust • Go • Python |
| **Corrode** | Threat Intelligence & Analytics | Rust • Python • PostgreSQL |

---

### 💻 Backend

![Rust](https://img.shields.io/badge/Rust-orange?logo=rust&logoColor=white)
![Go](https://img.shields.io/badge/Go-00ADD8?logo=go&logoColor=white)
![Python](https://img.shields.io/badge/Python-3776AB?logo=python&logoColor=white)
![Sanic](https://img.shields.io/badge/Sanic-222222?logo=python&logoColor=white)
![FastAPI](https://img.shields.io/badge/FastAPI-009688?logo=fastapi&logoColor=white)
![GIN](https://img.shields.io/badge/GIN-00ADD8?logo=go&logoColor=white)
![PostgreSQL](https://img.shields.io/badge/Postgres-336791?logo=postgresql&logoColor=white)
![MongoDB](https://img.shields.io/badge/MongoDB-47A248?logo=mongodb&logoColor=white)
![SQLite](https://img.shields.io/badge/SQLite-003B57?logo=sqlite&logoColor=white)
![Docker](https://img.shields.io/badge/Docker-2496ED?logo=docker&logoColor=white)

---

### 🖥️ Frontend

![TypeScript](https://img.shields.io/badge/TypeScript-3178C6?logo=typescript&logoColor=white)
![JavaScript](https://img.shields.io/badge/JavaScript-F7DF1E?logo=javascript&logoColor=black)
![React](https://img.shields.io/badge/React-20232a?logo=react&logoColor=61DAFB)
![Vue](https://img.shields.io/badge/Vue-35495E?logo=vue.js&logoColor=4FC08D)
![Svelte](https://img.shields.io/badge/Svelte-FF3E00?logo=svelte&logoColor=white)
![Angular](https://img.shields.io/badge/Angular-DD0031?logo=angular&logoColor=white)
![Nuxt](https://img.shields.io/badge/Nuxt-00C58E?logo=nuxt.js&logoColor=white)

---

### 🧰 Toolbox

![Biome](https://img.shields.io/badge/Biome-333333?logo=biome&logoColor=white)
![Knip](https://img.shields.io/badge/Knip-0A84FF?logo=checkmarx&logoColor=white)
![Linux](https://img.shields.io/badge/Linux-FCC624?logo=linux&logoColor=black)
![FreeBSD](https://img.shields.io/badge/FreeBSD-a60000?logo=freebsd&logoColor=white)
![Cloudflare](https://img.shields.io/badge/Cloudflare-F38020?logo=cloudflare&logoColor=white)
![AWS](https://img.shields.io/badge/AWS-232F3E?logo=amazonaws&logoColor=white)
![Azure](https://img.shields.io/badge/Azure-0078D4?logo=microsoftazure&logoColor=white)
![GitHub Actions](https://img.shields.io/badge/GitHub%20Actions-2088FF?logo=githubactions&logoColor=white)
![GraphQL](https://img.shields.io/badge/GraphQL-E10098?logo=graphql&logoColor=white)

---

### ⚡ GitHub Stats

![Stats](https://github-readme-stats.vercel.app/api?username=ul0gic&show_icons=true&theme=tokyonight&hide_border=true)
![Langs](https://github-readme-stats.vercel.app/api/top-langs/?username=ul0gic&layout=compact&theme=tokyonight&hide_border=true)

---

### 🧠 Motto
> “Security is not a feature — it’s an instinct.”
